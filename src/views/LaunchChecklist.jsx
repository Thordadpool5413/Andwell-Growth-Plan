...
const PHASE_COLORS = {
  ...
  "Phase 2 — Staged expansion (months 7–18)":  { dot: "bg-slate-400",  accent: "blue" },
  ...
  return (
    <div className="space-y-6">
      <SectionHeader eyebrow="Launch checklist" title="Phased launch task tracking">
        Track progress across launch phases. Click any status indicator to cycle it through Not Started → In Progress → Done. Progress is saved locally in your browser.
      </SectionHeader>
      ...
      <div className={`rounded-xl border p-5 ${dark ? "border-slate-700/60 bg-slate-800/60" : "border-slate-200 bg-white"}`}>
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className={`text-xs font-medium ${dark ? "text-slate-400" : "text-slate-500"}`}>Overall progress</p>
            <p className={`text-2xl font-bold tabular-nums ${dark ? "text-slate-100" : "text-slate-900"}`}>{done}/{total} complete</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            {hasOverrides && (
              <button
                onClick={resetAll}
                className={`rounded-lg px-3 py-1.5 text-xs font-medium ring-1 transition-colors duration-100 ${
            ...
            <div className={`text-sm font-semibold tabular-nums ${done === total ? "text-emerald-600" : dark ? "text-blue-400" : "text-blue-700"}`}>
      ...
      {phases.map((phase) => {
        ...
        const phaseInProg   = phaseItems.filter((i) => i.status === "In Progress").length;
        const phaseComplete = phaseDone === phaseItems.length;
        const phaseColors   = PHASE_COLORS[phase] || { dot: "bg-slate-400", accent: "slate" };
        return (
          <div key={phase}>
            <div className={`mb-3 flex items-center gap-3 rounded-lg border px-4 py-2.5 ${dark ? "border-slate-700/60 bg-slate-800/40" : "border-slate-200 bg-slate-50"}`}>
              <span className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${phaseColors.dot}`} />
              <div className="flex-1">
                <p className={`font-semibold text-sm ${dark ? "text-slate-100" : "text-slate-800"}`}>{phase}</p>
              </div>
              <div className={`flex items-center gap-3 text-xs ${dark ? "text-slate-400" : "text-slate-500"}`}>
                ...
                {phaseInProg > 0 && <span className={dark ? "text-blue-400" : "text-blue-600"}>{phaseInProg} in progress</span>}
                {phaseComplete && <span className="font-medium text-emerald-600">Phase complete</span>}
              </div>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border p-4 transition-colors duration-150 ${
                      isDone
                        ? dark ? "border-emerald-900/40 bg-emerald-950/15" : "border-emerald-100 bg-emerald-50/60"
                        : item.status === "In Progress"
                          ? dark ? "border-blue-900/40 bg-blue-950/15" : "border-blue-100 bg-blue-50/60"
                          : dark ? "border-slate-700/60 bg-slate-800/40" : "border-slate-100 bg-white"
                    }`}
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <p className={`font-semibold text-sm ${isDone ? (dark ? "text-slate-500 line-through" : "text-slate-400 line-through") : (dark ? "text-slate-100" : "text-slate-800")}`}>{item.county} County</p>
                          <ServiceBadge service={item.service} />
                          {isOverridden && (
                            <span className={`text-[10px] font-medium ${dark ? "text-blue-500" : "text-blue-600"}`}>Updated</span>
                          )}
                        </div>
                        <p className={`text-sm leading-6 ${isDone ? (dark ? "text-slate-500" : "text-slate-400") : (dark ? "text-slate-300" : "text-slate-600")}`}>{item.action}</p>
                      </div>
                      <div className={`flex-shrink-0 text-xs font-medium px-2 py-1 rounded-lg ${indicator.ring} ring-1 ${dark ? "text-slate-300" : "text-slate-600"}`}>
                        {item.status}
                      </div>
                    </div>
                  </div>
