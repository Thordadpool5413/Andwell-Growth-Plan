...
const STORAGE_KEY = "andwell-checklist-overrides";
...
export default function LaunchChecklist() {
  ...
  const resolvedItems = launchPlan.map((item) => ({
    ...item,
    status: overrides[item.task] ?? item.status,
  }));

  const phases = [...new Set(resolvedItems.map((item) => item.phase))];
  const done = resolvedItems.filter((i) => i.status === "Done").length;
  const total = resolvedItems.length;

  const toggleStatus = (task) => {
    setOverrides((prev) => {
      const currentStatus = prev[task] ?? launchPlan.find((i) => i.task === task)?.status ?? "Not Started";
      return { ...prev, [task]: nextStatus(currentStatus) };
    });
  };
  ...
  return (
    <div className="space-y-6">
      <SectionHeader eyebrow="Launch checklist" title="Phased launch task tracking">
        Track progress across launch phases. Click any task's status badge to cycle it through Not Started → In Progress → Done. Progress is saved locally in your browser. Use the Reset button to restore original status values.
      </SectionHeader>
      ...
      <div className={`rounded-3xl border p-5 ${dark ? "border-slate-700 bg-slate-800" : "border-slate-200 bg-white"}`}>
        ...
        <div className={`mt-3 h-2 w-full overflow-hidden rounded-full ${dark ? "bg-slate-700" : "bg-slate-100"}`}>
          <div className="h-full rounded-full bg-blue-600 transition-all duration-500" style={{ width: `${(done / total) * 100}%` }} />
        </div>
      </div>
      ...
      {phases.map((phase) => {
        ...
        return (
          <Card key={phase} title={phase} eyebrow={`Launch phase — ${phaseDone}/${phaseItems.length} done`}>
            <div className="space-y-3">
              {phaseItems.map((item) => {
                const isOverridden = overrides[item.task] !== undefined;
                return (
                  <div key={item.task} className={`flex items-start justify-between gap-4 rounded-2xl border p-4 transition ${dark ? "border-slate-700 bg-slate-700/30" : "border-slate-100 bg-slate-50"}`}>
                    ...
                    <button
                      onClick={() => toggleStatus(item.task)}
                      title="Click to advance status"
                      className="shrink-0 cursor-pointer transition-transform active:scale-95"
                      aria-label={`Toggle status for ${item.task}, currently ${item.status}`}
                    >
