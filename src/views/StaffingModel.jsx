  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {Object.entries(model.byService).map(([service, data]) => (
          <div key={service} className={`rounded-xl border p-5 ${dark ? "border-slate-700/60 bg-slate-800/60" : "border-slate-200 bg-white"}`}>
            <p className={`text-[11px] font-medium uppercase tracking-[0.1em] mb-1 ${dark ? "text-slate-500" : "text-slate-400"}`}>{service}</p>
            <p className={`text-sm font-medium mb-4 ${dark ? "text-slate-300" : "text-slate-600"}`}>{data.role}</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { year: "Y1", fte: data.y1.fte, barColor: "bg-blue-500" },
                { year: "Y2", fte: data.y2.fte, barColor: "bg-amber-500" },
                { year: "Y3", fte: data.y3.fte, barColor: "bg-emerald-500" },
              ].map(({ year, fte, barColor }) => (
                <div key={year} className={`rounded-lg p-3 text-center ${dark ? "bg-slate-700/40" : "bg-slate-50"}`}>
                  <div className={`mx-auto mb-2 h-0.5 rounded-full ${barColor}`} style={{ width: `${Math.min((fte / Math.max(data.y3.fte, 1)) * 100, 100)}%` }} />
                  <p className={`text-lg font-bold tabular-nums ${dark ? "text-slate-100" : "text-slate-900"}`}>{fte}</p>
                  <p className={`text-[10px] font-medium ${dark ? "text-slate-500" : "text-slate-400"}`}>{year} <Abbr term="FTE">FTE</Abbr></p>
                </div>
              ))}
            </div>
            <div className={`mt-3 pt-3 border-t text-sm ${dark ? "border-slate-700/60" : "border-slate-100"}`}>
              <div className="flex justify-between">
                <span className={dark ? "text-slate-400" : "text-slate-500"}>Y1 cost</span>
                <span className={`font-semibold tabular-nums ${dark ? "text-red-400" : "text-red-600"}`}>{currency(data.y1.cost)}</span>
              </div>
        ...
        <div className="relative">
          ...
          <div className={`overflow-x-auto rounded-lg border ${dark ? "border-slate-700/60" : "border-slate-200"}`}>
            <table className="w-full text-left text-sm">
              <thead className={`sticky top-0 z-10 text-xs uppercase tracking-wide ${dark ? "bg-slate-700/80 text-slate-400" : "bg-slate-50 text-slate-500 shadow-sm"}`}>
                <tr>
              ...
              <tbody className={`divide-y ${dark ? "divide-slate-700/60" : "divide-slate-100"}`}>
                {sortedFte.map((row, i) => (
                  <tr key={row.service} className={dark ? i % 2 === 1 ? "bg-slate-800/40 hover:bg-slate-700/40" : "hover:bg-slate-700/40" : i % 2 === 1 ? "bg-slate-50/60 hover:bg-slate-50" : "hover:bg-slate-50"}>
                    <td className={`px-5 py-4 font-semibold ${dark ? "text-slate-100" : "text-slate-800"}`}>{row.service}</td>
                    <td className={`px-5 py-4 ${dark ? "text-slate-300" : "text-slate-600"}`}>{row.role}</td>
                    <td className={`px-5 py-4 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{row.patientsPerFTE}</td>
                    <td className={`px-5 py-4 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{currency(row.avgSalary)}</td>
                    <td className={`px-5 py-4 text-right font-semibold tabular-nums ${dark ? "text-blue-400" : "text-blue-700"}`}>{row.y1fte}</td>
                    <td className={`px-5 py-4 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{row.y2fte}</td>
                    <td className={`px-5 py-4 text-right font-semibold tabular-nums ${dark ? "text-emerald-400" : "text-emerald-600"}`}>{row.y3fte}</td>
                    <td className={`px-5 py-4 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{currency(row.y1cost)}</td>
                    <td className={`px-5 py-4 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{currency(row.y3cost)}</td>
                    <td className={`px-5 py-4 text-right font-semibold tabular-nums ${dark ? "text-amber-400" : "text-amber-600"}`}>{currency(row.costPerStart)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className={`text-xs font-semibold border-t-2 ${dark ? "border-slate-600 bg-slate-700/40 text-slate-100" : "border-slate-200 bg-slate-50 text-slate-800"}`}>
                  <td className="px-5 py-3" colSpan={4}>Totals</td>
                  <td className={`px-5 py-3 text-right tabular-nums ${dark ? "text-blue-400" : "text-blue-700"}`}>{sortedFte.reduce((s, r) => s + r.y1fte, 0)}</td>
                  <td className={`px-5 py-3 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{sortedFte.reduce((s, r) => s + r.y2fte, 0)}</td>
                  <td className={`px-5 py-3 text-right tabular-nums ${dark ? "text-emerald-400" : "text-emerald-600"}`}>{sortedFte.reduce((s, r) => s + r.y3fte, 0)}</td>
                  <td className={`px-5 py-3 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{currency(sortedFte.reduce((s, r) => s + r.y1cost, 0))}</td>
                  <td className={`px-5 py-3 text-right tabular-nums ${dark ? "text-slate-300" : "text-slate-600"}`}>{currency(sortedFte.reduce((s, r) => s + r.y3cost, 0))}</td>
      ...
      <Card title="Staffing efficiency metrics" eyebrow="Cost intelligence">
        <div className="grid gap-3 md:grid-cols-3">
          {Object.entries(model.byService).map(([service, data]) => {
            ...
            return (
              <div key={service} className={`rounded-lg border p-4 ${dark ? "border-slate-700/60 bg-slate-700/20" : "border-slate-100 bg-slate-50"}`}>
                <p className={`font-semibold text-sm ${dark ? "text-slate-100" : "text-slate-800"}`}>{service}</p>
                <div className="mt-3 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className={dark ? "text-slate-400" : "text-slate-500"}>Y1 revenue</span>
                    <span className={`font-semibold tabular-nums ${dark ? "text-blue-400" : "text-blue-700"}`}>{currency(y1Rev)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold tabular-nums ${roi > 0 ? (dark ? "text-emerald-400" : "text-emerald-600") : (dark ? "text-red-400" : "text-red-600")}`}>{roi > 0 ? "+" : ""}{Math.round(roi)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={dark ? "text-slate-400" : "text-slate-500"}>Cost per start</span>
                    <span className={`font-semibold tabular-nums ${dark ? "text-slate-300" : "text-slate-700"}`}>{currency(data.y1.costPerStart)}</span>
                  </div>
                </div>
              </div>
            );
