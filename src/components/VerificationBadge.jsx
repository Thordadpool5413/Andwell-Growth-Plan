...
const STATUS_CONFIG = {
  ...
  "CMS matched in bundled source data": { bg: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300", dot: "bg-emerald-500", icon: "B" },
  "Bundled provider-file presence": { bg: "bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300", dot: "bg-blue-400", icon: "P" },
  "Source pending": { bg: "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300", dot: "bg-amber-400", icon: "?" },
...
export default function VerificationBadge({ status, showLabel = true, size = "sm" }) {
  ...
  const config = STATUS_CONFIG[safeStatus] || STATUS_CONFIG["Source pending"];
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded font-medium ${textSize} ${padding} ${config.bg}`}>
      ...
      {showLabel ? safeStatus : config.icon}
    </span>
  );
}
