...
export default function CustomTooltip({ active, payload, label, formatter }) {
  ...
  return (
    <div className={`rounded-xl border px-3 py-2.5 shadow-lg text-sm ${dark ? "border-slate-600 bg-slate-800 text-slate-100" : "border-slate-200 bg-white text-slate-900"}`}>
      {label !== undefined && (
        <p className={`mb-1.5 text-xs font-medium uppercase tracking-wide ${dark ? "text-slate-400" : "text-slate-500"}`}>{label}</p>
      )}
      {payload.map((entry, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold tabular-nums">{Array.isArray(val) ? val[0] : val}</span>
          </div>
        );
      })}
    </div>
